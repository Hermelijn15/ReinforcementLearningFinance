...
def increasing_gamma(episode, nr_episode, gamma_max = 0.9):
    #assymptotically increasing gamma
    
    gamma = (1-0.5**(10*episode/nr_episode))*gamma_max
    return gamma
...
def reverting(size, batch_size=50, learning_rate=0.01, risk_free_rate=0.0001, mean=0, std=1, gamma=0.9, folder = "random", features = {"Close": [-1],"portfolio value": [0]}):
    
    stock_data = pdg.reverting_prices(size, mean, std, reversion_strength=0.5)  
    #add random initial portfolio values
    stock_data.loc[:,"portfolio value"] = np.clip(np.random.normal(100,1,size)  , 0, None)
    
    
    output_dim = 5
    hidden_layers = np.array([128,128])
    gamma = gamma
    nr_steps = 100
...
def SP500( batch_size=50, learning_rate=0.01, risk_free_rate=0.0001, gamma=0.9, folder = "random", features = {"Close": [-1],"portfolio value": [0]}):
    
    stock_data = pdg.fetch_and_save_data("^GSPC")
    #add random initial portfolio values
    stock_data.loc[:,"portfolio value"] = np.clip(np.random.normal(100,1,len(stock_data))  , 0, None)
    
    
    output_dim = 5
    hidden_layers = np.array([8,8])
    gamma = gamma
    nr_steps = 100
...
if __name__ == "__main__":
    ...
    features = {"Close": [-1,-2,-3,-4,-5],"portfolio value": [0]}
    
    SP500(risk_free_rate=risk_free_rate, gamma =0.90, batch_size=batch_size , learning_rate = learning_rate , folder = "SP500_small",  features=features )
